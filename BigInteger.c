...
struct BigInteger div1(struct BigInteger num1, struct BigInteger num2) {
    ...
    while (current != NULL) {
        ...
        // Perform division while the remainder is greater than or equal to num2
        while (compareLists(remainder, num2) >= 0) {
            // Temporary variable for the quotient of this iteration
            struct BigInteger tempQuotient = initializeList();

            // Subtract num2 from the remainder
            struct BigInteger temp = sub(remainder, num2);
            freeList(remainder);
            remainder = temp;

            // Increment the last digit of the quotient
            struct Node* lastDigit = tempQuotient.head;
            struct Node* prev = NULL;
            ...
            // Check for carry in the quotient
            if (prev != NULL && prev->data > 9) {
                prev->data %= 10;
                insertDigit(tempQuotient, 1);
            }

            // Add the temporary quotient to the result
            result = add(result, tempQuotient);
            freeList(tempQuotient);
        }
